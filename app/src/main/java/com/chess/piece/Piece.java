...
/*    public void draw(Graphics2D g2){ //合作展现
        g2.drawImage(image,coordinate_x, coordinate_y,ChessBoard.SQUARE_SIZE,ChessBoard.SQUARE_SIZE,null);//图片来源 坐标 大小缩放 观察者 通常是null；
    }
    */
    ...
    public boolean isIntheBoard(int targetColumn,int targetRow){//理論上 這個判斷有可能要修改 地點。
        if(targetColumn >= 0 && targetColumn < ChessBoard.Max_Column && targetRow >= 0 && targetRow < ChessBoard.Max_Row){
            return true;
        }
        return false;
    }
  ...
  public int  get_Nearest_Ally_Obstacle_Position_On_The_Diagonal_Grid_Path(int targetColumn,int targetRow,String ColumnOrRow){
