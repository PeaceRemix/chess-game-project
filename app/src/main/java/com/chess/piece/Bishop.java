...
public class Bishop extends Piece {
    ...
    public boolean canMove(int targetColumn, int targetRow) {
        ...
        if (isIntheBoard(targetColumn, targetRow)) {
            ...
            if (isAnAllySquare(targetColumn, targetRow) == false) {
                ...
                if ((Math.abs(targetColumn - this.previous_Column) == Math.abs(targetRow - previous_Row))
                        && this.isASameSquare(targetColumn, targetRow) == false) {
    ...
    public boolean CanonRule_is_Valid(int targetColumn, int targetRow) {
        // 狀況一 不吃兵
        int Nearest_Ally_Obstacle_Position_Column = get_Nearest_Ally_Obstacle_Position_On_The_Diagonal_Grid_Path(
                targetColumn, targetRow, "Column");
        ...
        if (piece.there_Is_A_Obstacle_On_The_Diagonal_Grid_Path(targetColumn, targetRow) == false
                && piece.color.equals(this.color)) {
            if (isAnEmptySquare(targetColumn, targetRow)) {
                if ((targetRow - piece.previous_Row)
                        / Math.abs(targetRow - piece.previous_Row) == (targetRow - this.previous_Row)
                                / Math.abs(targetRow - this.previous_Row)) {
                    if ((targetColumn - piece.previous_Column)
                            / Math.abs(targetColumn - piece.previous_Column) == (targetColumn - this.previous_Column)
                                    / Math.abs(targetColumn - this.previous_Column)) {
                        if (Math.abs(targetColumn - piece.previous_Column) == 1
                                && Math.abs(targetRow - piece.previous_Row) == 1) {
                            return true;
                        }
