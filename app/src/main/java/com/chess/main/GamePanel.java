...
import java.awt.Graphics2D;//区别是啥
...
private void copyPieces(ArrayList<Piece> Beginning, ArrayList<Piece> Destination) {
        ...
        if(promotion){
             if(mouse.pressed){
                for(Piece piece: promoPieces){
                    ...
                    if(piece.column == mouse.coordinate_x/ChessBoard.SQUARE_SIZE && piece.row == mouse.coordinate_y/ChessBoard.SQUARE_SIZE && !CanChangeWithQueen){
                        switch(piece.type){
                            case Rook: simPieces.add(new Rook(currentColor,activePiece.column,activePiece.row));break;
                            case Knight: simPieces.add(new Knight(currentColor,activePiece.column,activePiece.row));break;
                            case Queen : simPieces.add(new Queen(currentColor,activePiece.column,activePiece.row));break;
                            case Bishop: simPieces.add(new Bishop(currentColor,activePiece.column,activePiece.row));break;
                            default : break;
                        }
                    ...
                    if(CanChangeWithQueen && piece.column == mouse.coordinate_x/ChessBoard.SQUARE_SIZE && piece.row == mouse.coordinate_y/ChessBoard.SQUARE_SIZE ){
                        switch(piece.type){
                            case Rook: simPieces.add(new Rook(currentColor,curPieceCol,curPieceRow));break;
                            case Knight: simPieces.add(new Knight(currentColor,curPieceCol,curPieceRow));break;
                            case Queen : simPieces.add(new Queen(currentColor,curPieceCol,curPieceRow));break;
                            case Bishop: simPieces.add(new Bishop(currentColor,curPieceCol,curPieceRow));break;
                            default : break;
                        }
            ...
            if(mouse.pressed ==false && activePiece != null){
                if(validSquare){//validSquare 可能有 邏輯重複的問題
                    ...
                    if(activePiece.type == Type.RookPawn && rightClick == true && RookPawnCanDifuse() == true){//click logic
    ...
    private void simulate(){//拖拉程式码 d
        ...
        if(activePiece.canMove(activePiece.column, activePiece.row)) {//檢查落點是否可走。
            ...
            if((activePiece.type == Type.Pawn && ((Pawn) activePiece).AEnpassantmove == false) || activePiece.type != Type.Pawn)
                activePiece.hittingP=activePiece.getHittingPiece(activePiece.column,activePiece.row);//更新
            
            else
                for(Piece piece : GamePanel.simPieces)
                    if(piece.column == activePiece.column && piece.row == activePiece.previous_Row && piece.color.equals(activePiece.color)==false && piece.twoStep == true)
    ...
    private boolean canPromote(){
        for(Piece piece : pieces){
            if(piece.type == Type.Pawn && ((piece.color == "white" && piece.row == 0) ||( piece.color == "black" && piece.row == 7))){
                promoPieces.clear();
    ...
    private boolean PawnCanFusionWithRook(){//可以往後走 所以 可以融合 條件限制寫在了pawn
        if(activePiece.type == Type.Pawn)
            if((currentColor == "white" && (activePiece.row > activePiece.previous_Row)) ||((currentColor == "black") && (activePiece.row < activePiece.previous_Row)))
                return true;
        return false;
    }
    ...
    private void ChangeWithQueen(int OrgC,int OrgR){
        ...
        switch(ChangePiece.get(0).type){
                case Rook: simPieces.add(new Rook(currentColor,OrgC,OrgR));break;
                case Knight: simPieces.add(new Knight(currentColor,OrgC,OrgR));break;
                case King : simPieces.add(new King(currentColor,OrgC,OrgR));break;
                case Bishop: simPieces.add(new Bishop(currentColor,OrgC,OrgR));break;
                case RookPawn: simPieces.add(new RookPawn(currentColor,OrgC,OrgR));break;
                case Pawn: simPieces.add(new Pawn(currentColor,OrgC,OrgR));break;
                case Queen: simPieces.add(new Queen(currentColor,OrgC,OrgR));break;
                default : System.out.println("wr");break;
        }
        copyPieces(simPieces,pieces);
    }
    

    public void paintComponent (Graphics g){//handle drawing component g automatik transfer form swing // 算是一种overide？？？
        ...
        for(Piece piece: simPieces)//initialize
            g2.drawImage(piece.image,piece.coordinate_x,piece.coordinate_y,ChessBoard.SQUARE_SIZE,ChessBoard.SQUARE_SIZE,null);
        ...
        if(activePiece != null){//可以删除 不影响运作
            if(canMove){
                ...
                g2.fillRect(activePiece.column*ChessBoard.SQUARE_SIZE,activePiece.row*ChessBoard.SQUARE_SIZE,ChessBoard.SQUARE_SIZE,ChessBoard.SQUARE_SIZE);
            }
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,1f));//设置绘画工具的透明度
            g2.drawImage(activePiece.image,activePiece.coordinate_x, activePiece.coordinate_y,ChessBoard.SQUARE_SIZE,ChessBoard.SQUARE_SIZE,null);
        ...
        if(promotion) {
            ...
            for(Piece piece : promoPieces)
                g2.drawImage(piece.image, piece.getCoordinate_x(piece.column), piece.getCoordinate_y(piece.row), ChessBoard.SQUARE_SIZE, ChessBoard.SQUARE_SIZE, null);
        }
    }
}